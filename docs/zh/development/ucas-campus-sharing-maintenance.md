...
>
> 最后核对：2026-08-06。分支、提交、镜像和生产进程都可能继续变化；维护时必须先读取实际 Git 与运行态，不能把示例当成当前状态。
...
1. `POST /admin/auth/password-reset/verification` 提交 `email`，成功受理时返回 `202`、`challengeToken` 和 `resendAfterSeconds`。
2. `POST /admin/auth/password-reset` 同时提交同一邮箱、`challengeToken`、六位 `verificationCode` 和 `newPassword`。

找回密码面向所有已激活的既有账户邮箱，不受校内注册域名限制，因此也支持既有 Owner 的 QQ 邮箱。邮箱会去除首尾空白并规范为小写；公开注册本身仍只允许上面的四个国科大域名。

验证码请求对存在与不存在的邮箱返回同样的 `202` 状态、消息和字段结构，不能从 HTTP 响应或页面判断账户是否存在。只有已激活的既有账户才会实际收到邮件；SMTP 发送和账户查询在公开请求路径之外执行。注册与找回密码分别使用 `registration` 和 `password_reset` purpose，并使用不同的摘要域，验证码不能跨用途使用。找回密码验证码受有效期、尝试次数和一次性消费约束；重置成功后，同一邮箱尚未消费的其他找回密码验证码也会失效。

重置成功会递增账户的 `auth_version`，使此前签发的所有浏览器 JWT 立即失效；前端必须清除本地会话并让用户重新登录。个人 API Key 是独立凭证，不会因找回密码自动失效；若怀疑 API Key 泄露，需要另行禁用或轮换。
...
1. 注册时确认邮箱属于允许域名；找回密码时确认邮箱是已激活既有账户的完整邮箱。两者都会规范化为小写。
2. `400` 表示请求格式、邮箱或新密码不合法；找回密码提交接口还会返回稳定的 `invalid_email`、`invalid_password` 或 `invalid_verification` 错误码。`invalid_verification` 同时涵盖验证码/挑战不匹配、过期、已消费或尝试次数耗尽，避免泄露账户状态。
3. `429` 表示重发冷却或邮箱、来源、全局小时限流；按响应/UI 的冷却时间等待，不要通过更换用途绕过限制。
4. `503` 表示验证码服务或异步投递队列在受理前不可用，应先检查系统密钥、执行器容量和服务日志；不要把它解释为“邮箱不存在”。
5. 请求返回 `202` 但未收到找回密码邮件时，只核对账户激活状态、SMTP 主机/端口/发件地址、`AXONHUB_SMTP_PASSWORD` 是否存在以及脱敏日志，不输出授权码、明文验证码或借机向请求者确认账户存在。异步 SMTP 失败会使该次未投递挑战失效，应在冷却后重新请求。
6. 邮件“能发出”不等于注册或重置成功；继续核对 purpose、验证码有效期、一次性消费、尝试次数，以及注册流程的项目初始化状态。
...
账户层不再设固定在途请求上限。渠道自身的容量控制仍作为上游保护独立存在，不能将它误解为用户并发配额。
...
配额按请求开始前的已结算用量检查，没有预占。单个大请求或多个并发请求可能使最终结算值越过边界；这些用量写入后，下一次新请求才会被阻止。维护时不能把它描述成严格的请求内硬截断。
...
- `429` 配额/限流直接进入跨渠道选择，不在同一渠道反复消耗重试预算。共享上游返回 `402` 时，该渠道立即进入有界的 5 分钟临时冷却，之后自动恢复探测；客户端收到限定语义的 `503 upstream_shared_quota_exhausted`，不再被误导成个人账单错误，但 execution 仍保留真实上游 `402`。
...
- Responses 流只有可解析的 `response.completed` 事件且 response snapshot 未声明失败或不完整时才是成功终态，传输层 `[DONE]` 本身绝不够；`response.incomplete` 及其真实原因必须保留，`response.failed`、`response.cancelled` 和顶层错误必须成为诚实失败，Token 用量或通用 finish reason 不能覆盖终态。
- 提交给用户前就失败的路由只保留 execution 排障信息，不抢占一个逻辑请求唯一的 UsageLog；最终真正交付的路由负责额度、钱包、排行和渠道归因。语义正文或工具调用一旦已经交付，后续不完整终态仍诚实记失败，但会结算其上报用量，因为这段输出已经被使用且不能透明重放。
...
| 配额 | 16M/64M 默认、Owner 修改实时全局生效、北京日/周边界、Key 配额先行、并发结算越界边界、同账户超过 4 个在途请求不再被账户并发拒绝 |
