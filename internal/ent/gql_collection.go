...
func (_q *ChannelQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "user":
			...
			_q.withUser = query
			if _, ok := fieldSeen[channel.FieldUserID]; !ok {
				selectedFields = append(selectedFields, channel.FieldUserID)
				fieldSeen[channel.FieldUserID] = struct{}{}
			}
		...
		case "expiresAt":
			if _, ok := fieldSeen[channel.FieldExpiresAt]; !ok {
				selectedFields = append(selectedFields, channel.FieldExpiresAt)
				fieldSeen[channel.FieldExpiresAt] = struct{}{}
			}
...
func (_q *UserQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "donatedChannels":
			...
			args := newChannelPaginateArgs(fieldArgs(ctx, new(ChannelWhereInput), path...))
			if err := validateFirstLast(args.first, args.last); err != nil {
				return fmt.Errorf("validate first and last in path %q: %w", path, err)
			}
			pager, err := newChannelPager(args.opts, args.last != nil)
			if err != nil {
				return fmt.Errorf("create new pager in path %q: %w", path, err)
			}
			if query, err = pager.applyFilter(query); err != nil {
			...
			if hasCollectedField(ctx, append(path, totalCountField)...) || hasCollectedField(ctx, append(path, pageInfoField)...) {
				...
				if hasPagination || ignoredEdges {
					...
					_q.loadTotal = append(_q.loadTotal, func(ctx context.Context, nodes []*User) error {
						...
						query.Where(func(s *sql.Selector) {
							s.Where(sql.InValues(s.C(user.DonatedChannelsColumn), ids...))
						})
						if err := query.GroupBy(user.DonatedChannelsColumn).Aggregate(Count()).Scan(ctx, &v); err != nil {
							return err
						}
						m := make(map[int]int, len(v))
						for i := range v {
					...
					_q.loadTotal = append(_q.loadTotal, func(_ context.Context, nodes []*User) error {
						for i := range nodes {
							n := len(nodes[i].Edges.DonatedChannels)
							if nodes[i].Edges.totalCount[2] == nil {
								nodes[i].Edges.totalCount[2] = make(map[string]int)
							}
							nodes[i].Edges.totalCount[2][alias] = n
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := entgql.LimitPerRow(user.DonatedChannelsColumn, limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
			}
			_q.WithNamedDonatedChannels(alias, func(wq *ChannelQuery) {
				*wq = *query
			})
